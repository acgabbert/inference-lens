...
Use the npm script rather than a bare `npx playwright test`: it probes loopback
before touching Playwright's browser cache, then uses the exact Chromium build
this Playwright version pins. When that executable is missing, it runs the
pinned installer with visible output and a bounded deadline. Playwright is
pinned to an exact version because a minor bump can expect a browser build
...
In a sandboxed or proxied environment, the pinned browser download can fail
outright — typically `403 request rejected: host not permitted` from
`cdn.playwright.dev` — and the suite never starts. That is not a reason to skip
the browser check. Environments that block the download should provide the
pinned browser through `$PLAYWRIGHT_BROWSERS_PATH` or a restored cache. Do not
point the committed suite at a nearby system Chromium.
